...
const app = express();
let posts = []; //1. 게시글 리스트로 사용할 posts(게시글)에 빈 리스트 할당

//req.body를 사용하려면 JSON 미들웨어를 사용해야 한다
//사용하지 않으면 undefined로 반환
app.use(express.json()); //2. JSON 미들웨어 활성화

//POST 요청 시 콘텐츠 타입이 application/x-www-form-urlencoded인 경우 파싱
...
app.post('/posts', (req, res) => {
  ...
  posts.push({ id: posts.length, title, name, text, createdDt: Date() });
...
app.delete('/posts/:id', (req, res) => {
  ...
  const isLengthChange = posts.length !== filteredPosts.length; //11. 글 삭제 확인
  posts = filteredPosts;
  if (isLengthChange) {
    //12. posts의 데이터 개수가 변경되었으면 삭제 성공
    res.json('OK');
    return;
  }
